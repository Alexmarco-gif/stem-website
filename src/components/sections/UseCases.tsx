...
const useCases = [
  {
    ...
    solution:
      "Stem monitors external activity, detects meaningful changes, and surfaces the shifts worth acting on.",
  ...
  },
  {
    ...
    title: "Policy & Risk",
    question:
      "What changes in policy or regulation should influence our next move?",
    pain: "Teams often hear about policy shifts too late or without enough context to interpret the impact.",
    solution:
      "Stem surfaces important policy and risk changes, connects them to monitored categories, and supports faster assessment.",
    ...
    <section className="py-24 bg-white">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          {/* Tab bar with scroll indicator on mobile */}
          <div className="relative">
            {/* Fade gradient on right edge for mobile scroll hint */}
            <div className="lg:hidden absolute right-0 top-0 bottom-4 w-12 bg-gradient-to-l from-white to-transparent pointer-events-none z-10" />

            <div className="flex lg:flex-col overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0 gap-2 lg:w-72 relative">
              {useCases.map((uc, i) => (
                <button
                  key={uc.id}
                  onClick={() => setActiveTab(uc.id)}
                  className={cn(
                    "relative px-6 py-4 rounded-xl text-left font-bold transition-all whitespace-nowrap lg:whitespace-normal overflow-hidden",
                    activeTab === uc.id
                ...
                >
                  {uc.title}
                  {/* Animated underline indicator for active tab (desktop) */}
                  {activeTab === uc.id && !reduced && (
                    <motion.div
                      layoutId="tab-indicator"
                      className="hidden lg:block absolute bottom-0 left-0 right-0 h-0.5 bg-white/40 rounded-full"
                  ...
                  >
                    ...
                    <div className="grid md:grid-cols-2 gap-12">
                      ...
                      <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm self-start">
                        ...
                        <p className="text-ink text-2xl font-bold leading-tight">
                          {uc.outcome}
                        </p>
                      </div>
                    </div>
